...
from src.catalogue.models.elasticsearch import PRODUCT_INDEX, ProductIndex, CATEGORY_INDEX, CategoryIndex
...
class ProductElasticManager:
    ...
    async def init_indices(self):
        products_index = Index(name=PRODUCT_INDEX, using=self.client)
    ...
    def build_product_search_query(keyword):
        search = Search(index=PRODUCT_INDEX).query(
            'multi_match', query=keyword, fields=['title', 'description', 'short_description']
        )
...
class CategoryElasticManager:
    ...
    async def init_indices(self):
        categories_index = Index(name=CATEGORY_INDEX, using=self.client)
    ...
    def build_category_search_query(keyword):
        search = Search(index=CATEGORY_INDEX).query(
