...
class CategoryService(BaseService[Category]):
    ...
    @staticmethod
    async def search(keyword: str):
        result = await (CategoryElasticManager()
                        .search_category(keyword=keyword))
        return result
    ...
    async def update_category_index(self, uuid):
        ...
        try:
            await (CategoryElasticManager()
                   .update_category_index(categories=categories))
        except ConnectionError as exc:
            await TaskStatusModel(uuid=uuid, status=TaskStatus.ERROR,
                                  details=str(exc)).save_to_redis()
